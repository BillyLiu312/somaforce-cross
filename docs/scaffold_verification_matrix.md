...
| Browser-visible noVNC desktop | `scripts/novnc_desktop.sh start --web-port 6080` | `novnc_started=1`, `vnc_password=...`, `display=:91`, and a browser URL for the forwarded web port are printed |
| GPU Xorg for noVNC attach | `scripts/start_gpu_xorg.sh start --display-num 0` | `xorg_started=1` or `xorg_already_reachable=1`; use `vulkaninfo --summary` to confirm headed Vulkan WSI support |
