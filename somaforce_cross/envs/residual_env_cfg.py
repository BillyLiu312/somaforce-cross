...
class SomaForceResidualEnvCfg(DirectRLEnvCfg):
    ...
    def __post_init__(self) -> None:
        ...
        if profile.runtime_mode not in ("c0", "scaffold_only", "residual"):
            raise ValueError(
                "runtime_mode must be 'c0', 'scaffold_only', or 'residual'"
            )
        if profile.runtime_mode == "c0":
        ...
        elif profile.runtime_mode == "scaffold_only":
            ...
            if profile.scaffold_stage not in ("C1", "C2", "C3"):
                raise ValueError("residual requires an explicit C1, C2, or C3 stage")
            if (
                not isinstance(profile.numeric_contract_path, str)
                or not profile.numeric_contract_path
            ):
                raise ValueError("residual requires an explicit v2 contract path")
